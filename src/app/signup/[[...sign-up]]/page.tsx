  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-[#09090b] overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-[-20%] left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full bg-[radial-gradient(circle,rgba(14,165,233,0.15)_0%,transparent_65%)] blur-[40px]" />
      ...
      <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-[rgba(9,9,11,0.85)] border-b border-white/[0.04]">
        <nav className="max-w-[1140px] mx-auto px-6 py-3.5 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5 text-white no-underline text-lg font-extrabold tracking-tight">
            <span className="w-8 h-8 rounded-[10px] bg-[#0ea5e9] inline-flex items-center justify-center">
          ...
          <div className="flex items-center gap-6">
            ...
            <Link href="/login" className="px-5 py-2 rounded bg-white/[0.06] text-[#fafafa] no-underline text-sm font-semibold hover:bg-white/[0.1] transition-colors">
              Login
            </Link>
          </div>
        </nav>
      ...
      <div className="relative z-10 flex flex-col items-center px-6 pt-24 pb-12 w-full max-w-md animate-[authSlideUp_0.5s_ease-out]">
        ...
        <div className="flex items-center gap-3.5 mb-3">
          <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-[#0ea5e9] to-[#0284c7] flex items-center justify-center shadow-[0_0_40px_rgba(14,165,233,0.35),0_0_80px_rgba(14,165,233,0.15)]">
        ...
        <p className="mt-8 text-[#52525b] text-xs text-center">
          Join teams using GetMention to scale their social presence
        </p>

        {/* Feature pills */}
        <div className="flex flex-wrap items-center justify-center gap-2 mt-5">
          {['6 Platforms', 'AI Replies', 'Auto-Post', 'Free Tier'].map((f) => (
            <span
              key={f}
              className="px-3 py-1 rounded-full text-[11px] font-medium text-[#71717a] bg-white/[0.04] border border-white/[0.06]"
            >
              {f}
            </span>
          ))}
